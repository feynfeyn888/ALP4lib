...
class Camera:
    ...
    def free_ran_mode(self):
        return ueye.is_SetExternalTrigger(self.h_cam, ueye.IS_SET_TRIGGER_SOFTWARE)
    ...
    def set_pixelclock(self):
        pixelclock = ueye.c_uint(24) #MHz
        #check(ueye.is_PixelClock(self.h_cam, ueye.IS_PIXELCLOCK_CMD_SET, pixelclock, ueye.sizeof(pixelclock)))
        return ueye.is_PixelClock(self.h_cam, ueye.IS_PIXELCLOCK_CMD_SET, pixelclock, ueye.sizeof(pixelclock))
        
    def get_pixelclock(self):
        ret = ueye.is_PixelClock(self.h_cam, ueye.IS_PIXELCLOCK_CMD_GET, pixelclock, ueye.sizeof(pixelclock))
        return ret

    def set_framerate(self):
        framerate = ueye.c_double(3.00) #fps
        check(ueye.is_SetFrameRate(self.h_cam, framerate, framerate))
        
    def set_exposure(self, t):
        # exposure = ueye.c_double(50.0) #ms
        exposure = ueye.c_double(t) #ms
        check(ueye.is_Exposure(self.h_cam, ueye.IS_EXPOSURE_CMD_SET_EXPOSURE, exposure, ueye.sizeof(exposure)))
        
        
        
    # def image_file(self,i):
    #     FileParams = ueye.IMAGE_FILE_PARAMS()
    #     FileParams.pwchFileName = "img"+str(i)+".png"
    #     #FileParams.pwchFileName = None
    #     FileParams.nFileType = ueye.IS_IMG_PNG
    #     FileParams.ppcImageMem = None
    #     FileParams.pnImageID = None
    #     ret = ueye.is_ImageFile(self.h_cam, ueye.IS_IMAGE_FILE_CMD_SAVE, FileParams, ueye.sizeof(FileParams))
    #     return ret

    
    def image_file(self, file_name):
        FileParams = ueye.IMAGE_FILE_PARAMS()
